...
''',
      ...
      if (!isLibrary) return;
      expect(g.inner(), List.generate(16, (_) => 2));
    });
...
''',
      ...
      final param = U8Array16(
        Uint8List.sublistView(Int32x4List.fromList([values])),
      );
      ...
      expect(
        ...
        Uint8List.sublistView(
          Int32x4List.fromList([values + Int32x4(42, 42, 42, 42)]),
        ),
      );
    });
...
''',
      ...
      final module = compileWasmModuleSync(
        binary,
        config: ModuleConfig(wasmtime: ModuleConfigWasmtime(wasmSimd: true)),
      );
      ...
      final param = U8Array16(
        Uint8List.sublistView(Float32x4List.fromList([values])),
      );

      // TODO: improve test for browser
      if (!isLibrary) return;
      expect(g.inner(42.2, param), [
        Uint8List.sublistView(
          Float32x4List.fromList([values * Float32x4(42.2, 42.2, 42.2, 42.2)]),
        ),
        42.20000076293945,
      ]);
    });
