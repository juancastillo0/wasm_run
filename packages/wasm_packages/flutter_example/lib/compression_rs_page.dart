...
class CompressionRsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final compressionRsLoader = Inherited.get<GlobalState>(
      context,
    ).compressionRs;
            ...
            return Column(
              children: [
                ...
                if (decompressed != null)
                  DownloadFileButton(
                    name: removeExtension(e.name, k.extensions),
                    bytes: decompressed,
                  )
                else
                  TextButton(
              ...
              ...[
                DropdownButtonFormField(
                  ...
                  onChanged: (v) => e.zipOptions.value = e.zipOptions.value
                      .copyWith(compressionMethod: v),
                ),
                IntInput(
                  label: 'Permissions',
                  initialValue: e.zipOptions.value.permissions,
                  onChanged: (v) => e.zipOptions.value = e.zipOptions.value
                      .copyWith(permissions: Some(v)),
                ),
                IntInput(
                  label: 'Modified Time',
            ...
            children: [
              ...[
                ...
                IntInput(
                  ...
                  onChanged: (v) => e.tarHeader.value = e.tarHeader.value
                      .copyWith(deviceMajor: Some(v)),
                ),
                IntInput(
                  label: 'Device Minor',
                  initialValue: e.tarHeader.value.deviceMinor,
                  onChanged: (v) => e.tarHeader.value = e.tarHeader.value
                      .copyWith(deviceMinor: Some(v)),
                ),
              ].map((e) => e is Container ? e : e.container(width: colWidth)),
            ],
          );
                        ...
                        Expanded(
                          child: SingleChildScrollView(
                            child: Column(
                              children: [
                                ...(switch (state.view) {
                                  CompressorView.compress => state.files.map(
                                    compressFileView,
                                  ),
                                  CompressorView.zip => state.files.map(
                                    zipFileView,
                                  ),
                                  CompressorView.tar => state.files.map(
                                    tarFileView,
                                  ),
                                }),
                                ErrorMessage(
